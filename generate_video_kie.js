// generate_video_kie.js
// PRODUCTION video generator — replaces both generate_video_higgsfield.js
// (Higgsfield) and generate_tiktok.js (fal.ai/Kling) as the sole video
// engine for daily-offer.yml, per decision made after comparing samples
// (Sep 2026): Kie.ai's Kling 3.0 produced better, more reliable results
// than Higgsfield's undocumented/guessed request shape.
//
// Voice generation is UNCHANGED and still runs through fal.ai's ElevenLabs
// wrapper — Kie.ai has no standalone text-to-speech product, so fal.ai is
// kept for that one job only. Everything else (captions, watermark, music
// mixing, scene caching) is carried over unchanged from
// generate_video_higgsfield.js.
//
// Output: today_tiktok.mp4 (same filename/contract as before)
// Requires: assets/mascot.png committed to the repo
// Requires env: FAL_KEY (voice), KIE_API_KEY (video)
//
// ERROR LOGGING: on any failure this script writes video_error.json with
// structured diagnostic detail (message, HTTP status if present, the raw
// response body if present, and which stage failed — voice generation vs
// Kie.ai animation) and prints the same to the console. daily-offer.yml
// reads this to decide whether to fail loudly.
...
const CACHE_DIR = path.join(process.cwd(), ".cache", "tiktok_kie");
...
function writeErrorLog(stage, sceneIndex, e) {
  ...
  const payload = {
    ...
    stage, // "voice_generation" | "kie_animation" | "unknown"
  ...
  console.error("=== Kie.ai pipeline failure detail ===");
...
// --- Caption helpers (unchanged from generate_video_higgsfield.js) --------
...
// --- Scene cache (unchanged pattern from generate_video_higgsfield.js) ----
...
function loadFromCache(key) {
  ...
  if (!fs.existsSync(videoPath) || !fs.existsSync(audioPath) || !fs.existsSync(metaPath)) return null;
  try {
    const meta = JSON.parse(fs.readFileSync(metaPath, "utf8"));
    return { videoPath, audioPath, audioDuration: meta.audioDuration, text: meta.text, wordTimestamps: meta.wordTimestamps || null };
  } catch (_) {
    return null;
  }
}
...
async function waitForKieVideo(taskId, { timeoutMs = 8 * 60 * 1000, intervalMs = 5000 } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (data.state === "success") {
      ...
      console.log(`Kie.ai video complete — ${data.creditsConsumed ?? "?"} credits consumed.`);
      return urls[0];
    }
    if (data.state === "fail") {
    ...
    console.log(`  Kie.ai: ${data.state} (progress ${data.progress ?? "?"}%, ${elapsed}s elapsed)...`);
...
async function generateSceneFresh(line, sceneIndex, tmpDir) {
  ...
  console.log(`Scene ${sceneIndex}: animating mascot via Kie.ai (billed; audio ${audioDuration.toFixed(1)}s, requesting ${durationStr}s)...`);
  let videoUrl;
  try {
    const prompt =
    ...
    videoUrl = await waitForKieVideo(taskId);
  } catch (e) {
    writeErrorLog("kie_animation", sceneIndex, e);
    throw new Error(`Kie.ai animation failed on scene ${sceneIndex}: ${e.message}`);
  }
...
async function main() {
  ...
  const tmpDir = "/tmp/tiktok_kie_frames";
  ...
  console.log("Wrote today_tiktok.mp4 (Kie.ai style)");
}
