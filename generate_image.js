...
//
// UPDATED (Sep 2026): the mascot panel now shows a unique, topic-relevant
// image generated by Kie.ai for each day's specific product (referencing
// the SHA mascot so it stays on-brand) instead of the same static mascot
// crop every day. If Kie.ai fails for any reason — no API key, timeout,
// task failure — this falls back to the original static mascot crop so the
// daily post never breaks; this is a nice-to-have, not worth failing a
// day's post over.
...
// --- Kie.ai per-day image generation (new) -----------------------------
...
/**
 * Returns a Buffer of a panel-ready image (any size — cropped to fit
 * PANEL_W x PANEL_H by the caller). Tries Kie.ai first for a unique,
 * topic-relevant image built from today's actual product; falls back to
 * the static mascot crop on ANY failure, logging why, so the day's post
 * always gets an image either way.
 */
async function getPanelImageBuffer(p) {
  if (!process.env.KIE_API_KEY) {
    console.log("KIE_API_KEY not set — using the static mascot image (as before).");
    return fs.readFileSync(MASCOT_PATH);
  }
  try {
    console.log("Generating a unique Kie.ai image for today's offer card...");
    const referenceUrl = await uploadMascotToKie();
    const headline = p.graphic_headline || p._meta.product;
    const subline = p.graphic_subline || "";
    const prompt =
      `A warm, professional illustrated social media card scene for a digital education ` +
      `business called Smarter Hustle Academy, visually representing today's featured offer: ` +
      `"${headline}" ${subline}. Forest green (#2D6A4F) and gold (#D4A017) color scheme, cream ` +
      `background. Feature the mascot character prominently, doing something visually connected ` +
      `to the offer topic. Clean flat illustration style, motivational and trustworthy feel.`;
...
async function main() {
  ...
  // Get today's panel image — Kie.ai if available, static mascot crop as
  // the safe fallback (see getPanelImageBuffer above).
  const panelImageBuffer = await getPanelImageBuffer(p);
  const mascotCropped = await sharp(panelImageBuffer)
